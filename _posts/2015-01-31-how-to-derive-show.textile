...
This is not the end however, since this does not mention the name of the structure we are trying to derive @Show@ for. This must take place in @trait_def.expand()@. This function expands the trait definition, ensuring that the derived-upon item is either a struct or an enum, taking care of various

Ultimately it ends in the following item creation:
{% highlight rust %}
cx.item(
    self.span,
    ident,
...
which may not mean a lot, but it's there. We can conclude that we end up calling @cx.item@, creating a new item in the AST. The @item()@ method is not defined on @ExtCtxt@ itself, but rather declared in a trait @AstBuilder@, which is implemented of @ExtCtxt@. Ultimately @cx.item()@ expands to:
